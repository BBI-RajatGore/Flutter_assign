...
void main() {
  ...
  group('fetchNews', () {
    test('should return a list of NewsArticle when the response is successful', () async {
      ...
      print('$baseUrl?q=${Uri.encodeQueryComponent(query)}&language=$language&sortBy=$sortBy&apiKey=$apiKey&page=$page&pageSize=$pageSize');

      // Verify that http.get was called with the correct URL
      verify(() => mockHttpClient.get(
        Uri.parse(
          '$baseUrl?q=${Uri.encodeQueryComponent(query)}&language=$language&sortBy=$sortBy&apiKey=$apiKey&page=$page&pageSize=$pageSize',
        ),
      )).called(1);
    ...
    test('should throw an exception when the status code is not 200', () async {
      ...
      when(() => mockHttpClient.get(any())).thenAnswer(
        (_) async => http.Response(jsonEncode(mockErrorResponse), 401), // Simulating a 401 Unauthorized error
      );
    ...
    test('should throw an exception when an error occurs during the HTTP request', () async {
...
//     mockHttpClient = MockHttpClient(); // Use the generated mock class
//     dataSource = NewsRemoteDataSourceImpl(client: mockHttpClient);
//   });
